# What is AI Canva? — A Beginner's Guide

> **AI Canva** is a visual whiteboard where you build AI "pipelines" — drop boxes on a canvas, connect them, and let AI content flow from box to box.

---

## 1. The big idea

Most AI apps are single-purpose chat boxes. AI Canva turns AI into a **visual, modular pipeline**. Think of it as a flowchart where each node is a tool:
...
Instead of writing one giant prompt, you **compose smaller steps** and pass each output to the next box. This teaches a core AI principle: **breaking a hard problem into smaller steps and chaining them** (often called "pipelining" or "agentic workflows").
...
**Two kinds of boxes:** **Input boxes** (no AI) start the data — like an Idea or an Image. **Worker boxes** (AI) transform it — like Research or Code. Boxes are defined in `client/src/types.ts` and rendered by `client/src/components/BoxNode.tsx`.
...
3. The client gathers the connected boxes' outputs and fills the box's **prompt template** (variables like `{{input_1}}`, `{{Box Name}}`, `{{inputs}}`).
...
The heart of this logic lives in `client/src/store/boardStore.ts` (the `runBox` function) and `client/src/lib/prompts.ts` (template filling).
...
## 6. Tech stack & what you'll learn
...
## 7. AI providers (important for running it)
...
> **For students:** you can run AI **for free** by running Ollama on your own computer and pointing the app at `http://localhost:11434`. No paid key required for text.

---

## 8. Why this is a great training project

Working on AI Canva lets you touch a **real, deployed, multi-service app** — not a toy. Depending on your focus, you can learn:

- **Frontend:** React components, hooks, a single central store, TypeScript, Tailwind.
- **AI & prompting:** prompt templates, system prompts, chaining outputs, structured output (parsing JSON from an LLM for slides/code).
...
> **In short:** AI Canva is a visual way to build AI workflows — and a real codebase you can learn from, extend, and contribute to. Your project will add your own boxes to it.
